...
def validate_checkpoint_contract(checkpoint: dict[str, object]) -> None:
    if int(checkpoint.get("input_size", -1)) != len(FEATURE_NAMES):
        raise ValueError(
            "checkpoint input size does not match the Playground feature contract"
        )
    if tuple(checkpoint.get("feature_names", ())) != FEATURE_NAMES:
        raise ValueError(
            "checkpoint feature names do not match the Playground feature contract"
        )
    if tuple(checkpoint.get("labels", ())) != LABELS:
        raise ValueError(
            "checkpoint labels do not match the Playground action contract"
        )
